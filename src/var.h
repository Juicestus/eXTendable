...
enum VAR_FLAGS {
    ...
    VAR_NATIVE      = 128, 
    VAR_NUMERICMASK = VAR_NULL |
                      VAR_DOUBLE |
                      VAR_INTEGER,
    VAR_VARTYPEMASK = VAR_DOUBLE |
                      VAR_INTEGER |
                      VAR_STRING |
                      VAR_FUNCTION |
                      VAR_OBJECT |
...
public:
    ...
    Link *findChildOrCreate(const std::string &childName, int varFlags = VAR_UNDEFINED); 
    ...
    bool isNumeric() { return (flags&VAR_NUMERICMASK) != 0; }
    ...
    bool isUndefined() { return (flags & VAR_VARTYPEMASK) == VAR_UNDEFINED; }
